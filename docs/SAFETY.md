...
- **BlackboardEntryTtlEvictor**: TTL eviction is local memory hygiene only; never executes tools or contacts providers.
...
- `geohash`: encodes lat/lon to geohash or decodes geohash to lat/lon (`mode`: `encode` default or `decode`; precision 1..12; `lat`/`lon` or `latitude`/`longitude`; `geohash`/`text`/`hash` for decode); rejects out-of-range coords/precision and invalid alphabet; never executes code or makes network requests.
...
- `jaro_winkler`: returns classic Jaro-Winkler similarity score 0..1 between `a` and `b` (max 2000 chars each); companion to `levenshtein`; never executes code or makes network requests.
...
- `braille`: encodes ASCII text to Unicode Braille (U+2800 block) or decodes Braille back to ASCII (`mode`: `encode` default or `decode`; `text` or `data`; max 2000 chars); rejects empty/oversized/non-ASCII encode and non-Braille decode; never executes code or makes network requests.
